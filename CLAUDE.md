...
- **XML Context Formatting**: Retrieved documents are formatted as structured XML with essential metadata (source_authority, urgency, disaster_type, url) for better LLM processing
...
- **Multi-Stage Filtering**: Similarity threshold of 0.995 combined with profile-based metadata filters for improved precision
- **Clean User Interface**: System prompts and RAG context hidden from users, showing only clean conversation flow with citation support
...
- `src/services/context_formatter.py`: XML formatter for retrieved documents with metadata attributes
...
3. **Intelligent Document Retrieval**: 
   - Queries ChromaDB for top 7 most similar documents with automatic metadata filtering
   - Profile-based filtering (victim: urgent response info, resident: prevention/preparation, family: contacts/recovery)
   - Filters results by distance threshold (< 0.995) and relevance scoring
   - Formats retrieved documents as XML with essential metadata attributes (source_authority, urgency, disaster_type, url)
   - XML format enables structured citations and better LLM comprehension
